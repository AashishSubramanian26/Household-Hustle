...
public:
	...
	UPROPERTY(config, VisibleAnywhere, BlueprintReadOnly, Category = "Runtime Vertex Paint and Detection Plugin", meta = (ToolTip = "Which Optimization Data Asset we should use when painting. Meshes that are registered in it are only painted on those amounts of LODs which can save alot of performance. \nThis can be changed in the Editor Widget. "))
		TSoftObjectPtr<UVertexPaintOptimizationDataAsset> vertexPaintAndDetectionPlugin_OptimizationDataAssetToUse = nullptr;

	UPROPERTY(config, VisibleAnywhere, BlueprintReadOnly, Category = "Runtime Vertex Paint and Detection Plugin", meta = (ToolTip = "Which Vertex Paint Material Data Asset to use. This is the data asset you register your materials that use vertex colors that you want to detect what is on what channel etc. \nIf there is none set then you can't detect vertex data, you can only paint. \nThis can be changed in the Editor Widget. "))
	...
	UPROPERTY(config, EditDefaultsOnly, Category = "Runtime Vertex Paint and Detection Plugin", meta = (ToolTip = "If we should Show Notifications when making changes in the Editor Widget. "))
		bool vertexPaintAndDetectionPlugin_EditorWidgetNotificationEnabled = true;

	UPROPERTY(config, EditDefaultsOnly, Category = "Runtime Vertex Paint and Detection Plugin", meta = (ToolTip = "Duration of the Notifications that pop up when making certain changes in  the editor widget. "))
	...
	UPROPERTY(config, EditDefaultsOnly, Category = "Runtime Vertex Paint and Detection Plugin", meta = (ToolTip = "This is the Max Amount of Tasks a Mesh can have, any new Tasks will Declined. Don't recommend changing this too much since if the Task Queue grows to large, performance will be affected since the TMaps become much more heavier when you run .Add, .FindRef etc. For instance at 1000 Tasks you can drop to 20FPS. \nRecommend making sure you don't add more Tasks than you need, for instance adding the new task when the current task is finished instead of every frame. If the queue grows too large, paint jobs will feel unresponsive as well since it may take a while before it reaches the latest added task. "))
		int vertexPaintAndDetectionPlugin_MaxAmountOfAllowedTasksPerMesh = 15;

	UPROPERTY(config, EditDefaultsOnly, Category = "Runtime Vertex Paint and Detection Plugin", meta = (ToolTip = "The Priority of the Thread Pool we're creating for Painting/Detecting. "))
	ETVertexPaintThreadPriority vertexPaintAndDetectionPlugin_MultithreadPriority = ETVertexPaintThreadPriority::Highest;
	...
	bool vertexPaintAndDetectionPlugin_UseMaximumAmountOfCoresForMultithreading = false;
