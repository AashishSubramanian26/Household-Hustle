...
void UVertexPaintClothOverlapComponent::DeActivateClothOverlapTracing() {
	...
	for (auto& cachedOverlappCompTMapTemp : clothOverlappingActorsAndComponent) {
		...
		for (int i = 0; i < cachedOverlappCompTMapTemp.Value.overlappingPrimitiveComponents.Num(); i++) {

			if (UKismetSystemLibrary::DoesImplementInterface(cachedOverlappCompTMapTemp.Value.overlappingPrimitiveComponents[i]->GetOwner(), UVertexPaintDetectionInterface::StaticClass()))
				IVertexPaintDetectionInterface::Execute_ClothEndOverlappingMesh(cachedOverlappCompTMapTemp.Value.overlappingPrimitiveComponents[i]->GetOwner(), cachedOverlappCompTMapTemp.Value.overlappingPrimitiveComponents[i], GetOwner(), parentSkeletalMeshComponent, cachedOverlappCompTMapTemp.Value.overlappingItems[i]);
		}
	}
...
void UVertexPaintClothOverlapComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction) {
	...
	FVertexPaintClothOverlapComponentsStruct clothCurrentOverlappingComponentsFromTrace_Local;
	...
	for (auto& currentClothDataTemp : clothingData_Local) {
		...
		for (int i = 0; i < currentClothDataTemp.Value.Positions.Num(); i += FMath::Clamp(clothVertexTraceIntervall, 1, clothVertexTraceIntervall)) {
			...
			clothCurrentOverlappingComponentsFromTrace_Local.overlappingPrimitiveComponents.Empty();
			clothCurrentOverlappingComponentsFromTrace_Local.overlappingItems.Empty();
				...
				if (clothHitResult_Local.Component.Get()) {
					...
					if (clothHitResult_Local.Component.Get()->GetCollisionResponseToChannel(parentSkeletalMeshComponent->GetCollisionObjectType()) == ECollisionResponse::ECR_Overlap) {
						...
						if (clothCurrentOverlappingActorsAndComponents_Local.Contains(clothHitResult_Local.Actor.Get()))
							clothCurrentOverlappingComponentsFromTrace_Local = clothCurrentOverlappingActorsAndComponents_Local.FindRef(clothHitResult_Local.Actor.Get());

						clothCurrentOverlappingComponentsFromTrace_Local.overlappingPrimitiveComponents.Add(clothHitResult_Local.Component.Get());
						clothCurrentOverlappingComponentsFromTrace_Local.overlappingItems.Add(clothHitResult_Local.Item);

						FVertexPaintClothOverlapComponentsStruct clothCurrentOverlappingComponentsFromTraceStruct_Local;
						clothCurrentOverlappingComponentsFromTraceStruct_Local = clothCurrentOverlappingComponentsFromTrace_Local;
						...
						if (clothCurrentOverlappingActorsAndComponents_Local.Contains(clothHitResult_Local.GetActor()))
							clothCurrentOverlappingComponentsFromTrace_Local = clothCurrentOverlappingActorsAndComponents_Local.FindRef(clothHitResult_Local.GetActor());
	...
	for (auto& cachedOverlappCompTMapTemp : clothOverlappingActorsAndComponent) {
		...
		for (int i = 0; i < cachedOverlappCompTMapTemp.Value.overlappingPrimitiveComponents.Num(); i++) {
			...
			totalOverlappedCachedComponents_Local.Add(cachedOverlappCompTMapTemp.Value.overlappingPrimitiveComponents[i]);

			// If current overlapped components doesn't contain the cached one, then runs End Overlap on the cached one
			if (!clothCurrentOverlappingComponentsTotal_Local.Contains(cachedOverlappCompTMapTemp.Value.overlappingPrimitiveComponents[i]))
	...
	for (auto& currentOverlappCompTMapTemp : clothCurrentOverlappingActorsAndComponents_Local) {
		...
		for (int i = 0; i < currentOverlappCompTMapTemp.Value.overlappingPrimitiveComponents.Num(); i++) {
			...
			if (totalOverlappedCachedComponents_Local.Num() > 0) {
				...
				if (!totalOverlappedCachedComponents_Local.Contains(currentOverlappCompTMapTemp.Value.overlappingPrimitiveComponents[i]))
			...
			if (runBeginOverlap_Local) {
				...
				if (UKismetSystemLibrary::DoesImplementInterface(currentOverlappCompTMapTemp.Value.overlappingPrimitiveComponents[i]->GetOwner(), UVertexPaintDetectionInterface::StaticClass()))
					IVertexPaintDetectionInterface::Execute_ClothBeginOverlappingMesh(currentOverlappCompTMapTemp.Value.overlappingPrimitiveComponents[i]->GetOwner(), currentOverlappCompTMapTemp.Value.overlappingPrimitiveComponents[i], GetOwner(), parentSkeletalMeshComponent, currentOverlappCompTMapTemp.Value.overlappingItems[i]);
			}
		}
	}
